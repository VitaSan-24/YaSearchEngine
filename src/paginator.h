...
//    std::ostream& operator<<(std::ostream& os, const IteratorRange<Iterator>& document_range) {
//
//        for (Iterator it = document_range.begin(); it != document_range.end(); ++it){
//                std::cout << *it;
//                }
//
//        return os;
...
explicit Paginator(const Iterator& p_begin, const Iterator& p_end, size_t page_size)  {
        ...
        if (page_size == 0){
            pages_(IteratorRange(p_begin, p_end));
            return;
        }
        ...
        for (size_t left = distance(p_begin_tmp, p_end_tmp); left > 0;) {
            ...
            const Iterator current_page_end = next(p_begin_tmp, current_page_size);
